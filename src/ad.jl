...
function jacpt_vec!(
    ...
    @assert length(vec) == length(z)

    x = @view z[1:diff_dim]
    y = @view z[diff_dim+1:diff_dim+alg_dim]
    v = @view z[diff_dim+alg_dim+1:end]

    vecx = @view vec[1:diff_dim]
    vecy = @view vec[diff_dim+1:diff_dim+alg_dim]
    ...
    @inbounds for (i, device) in enumerate(sys.dynamic.devices)
        ...
        result = @view out[par_ptr:par_ptr+par_size-1]
        λ = @views vec[dev + 2*bus - 1: dev + 2*bus]

        J = ForwardDiff.jacobian(cinj, par)
        result .+= transpose(J)*λ
        ...
        if alg_size > 0
            result = @view out[par_ptr:par_ptr+par_size-1]
            λ = @views vecy[alg_ptr:alg_ptr+alg_size-1]
        ...
        if diff_size > 0
            result = @view out[par_ptr:par_ptr+par_size-1]
            λ = @views vecx[diff_ptr:diff_ptr+diff_size-1]

...
@testset "JFNK per-scenario residual matches batched (≤ 1e-14)" begin
    HAS_CUDA_14c || return
    ...
    gbl = ext.GpuBatchedLayout(dp, ps, M)
    ext._residual_all_scenarios_gpu!(gbl, dyn, L)
    f_batched = Array(gbl.f)

    # Per-scenario 1D residual
    sys_dim = gbl.sys_dim
    u_dim = size(gbl.u, 2)
    p_dim = size(gbl.p, 2)
    for m in 1:M
        z_1d = CUDA.zeros(Float64, sys_dim)
        u_1d = CUDA.zeros(Float64, u_dim)
        p_1d = CUDA.zeros(Float64, p_dim)
        inj_1d = CUDA.zeros(Float64, 2 * gbl.inj_meta_n_total)
        f_1d = CUDA.zeros(Float64, sys_dim)
        ext._extract_row_range!(z_1d, gbl.z, m, 1, sys_dim)
        ext._extract_row_range!(u_1d, gbl.u, m, 1, u_dim)
        ext._extract_row_range!(p_1d, gbl.p, m, 1, p_dim)
        ext._residual_one_scenario_gpu!(f_1d, z_1d, u_1d, p_1d, inj_1d, gbl, dyn, L)
        f_1d_cpu = Array(f_1d)
        @test maximum(abs, f_1d_cpu - f_batched[m, :]) <= 1e-14
    end
end

@testset "JFNK GPU trajectory matches CPU (≤ 1e-10)" begin
    HAS_CUDA_14c || return
    ...
    gbl = ext.GpuBatchedLayout(dp2, ps, 1)
    tvec_gpu, trajs_gpu = ext.integrate_gpu_jfnk!(gbl, ps, 0.5; dt=1.0/120.0,
                                                     newton_tol=1e-10, itermax=10)

    # JFNK uses finite-difference Jacobian approximation, so ~1e-9 error expected
    @test maximum(abs, trajs_gpu[1] - traj_ref) <= 1e-8
end
